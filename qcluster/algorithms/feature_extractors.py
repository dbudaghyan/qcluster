...
def pacmap_reduction(embeddings: torch.Tensor,
                     n_components: int = 2,
                     n_neighbors=10,
                     mn_ratio=0.5,
                     fp_ratio=2.0) -> torch.Tensor:
    """
    ...
    Returns:
        A torch.Tensor containing the reduced embeddings.
    """
    pacmap_model = PaCMAP(n_components=n_components, n_neighbors=n_neighbors,
                          MN_ratio=mn_ratio, FP_ratio=fp_ratio, verbose=True)
    reduced_embeddings = pacmap_model.fit_transform(embeddings.cpu().numpy(), init='pca')
    return torch.tensor(reduced_embeddings)
